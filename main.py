...
UNIT_CONVERSION = {'portland': 0.3048, 'san_francisco': 1.609344}  # Convert Length Units from foot and miles.


def make_whole_dataframe(data_dict: Path, filename: str = 'cities.csv', calc_times: bool = False) -> DataFrame:
    """
    Merges numeric Data from OSM, speed calculations and entropy / orientation-order-index
    :param data_dict: Path to data directory
    :param filename: Filename of main input file (cities.csv)
    :param calc_times: if True: recalculate travel times from GTFS-Data
    ...
    full_data: DataFrame = full_data.merge(right=orientations, right_on=['Stadt'], left_on=['Stadt'])
    full_data.drop(axis='columns', inplace=True,
                   labels=['Ost', 'West', 'Nord', 'Sued', 'RailModes', 'Geschwindigkeit', 'GTFS-Daten',
                           'Ignore_LineNumber'])
